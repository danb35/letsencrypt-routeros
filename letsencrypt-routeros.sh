...
if [[ -z "$ROUTEROS_USER" ]] || [[ -z "$ROUTEROS_HOST" ]] || [[ -z "$DOMAIN" ]]; then
        echo "Check the config file $CONFIG_FILE and ensure all required parameters are set."
...
if [[ ! $RESULT == 0 ]]; then
        echo -e "\nError in: $routeros"
        echo "More info: https://wiki.mikrotik.com/wiki/Use_SSH_to_execute_commands_(public/private_key_login)"
        exit 1
else
